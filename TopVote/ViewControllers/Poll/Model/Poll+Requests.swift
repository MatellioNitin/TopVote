...
extension Poll {
    ...
    static func getPollDeepLink(pollID: String, error: @escaping (_ errorMessage: String) -> Void, completion: @escaping (_ poll: Poll) -> Void) {
        Poll.provider.request(Poll.API.getPollDeepLink(getPollId: pollID)) { result in
    ...
    static func getLeaderboardDetail(pollID: String, error: @escaping (_ errorMessage: String) -> Void, completion: @escaping (_ poll: Poll) -> Void) {
        Poll.provider.request(Poll.API.getPollleaderboardDetail(getPollId: pollID)) { result in
            result.handleResponseData(completion: { (errorMessage, data, token) in
                if let value = data {
                    let pollObj: Poll = Poll.create(data: value)!
                    //  let pollObj: Poll = Poll.models(data: value)
