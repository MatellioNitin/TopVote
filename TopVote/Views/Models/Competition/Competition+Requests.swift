...
extension Competition {
    ...
    static func findPrivatePoll(queryParams: [String: Any]?, error: @escaping (_ errorMessage: String) -> Void, completion: @escaping (_ competitions: Competitions) -> Void) {
        Competition.provider.request(Competition.API.privatePoll(queryParams: queryParams)) { result in
            result.handleResponseData(completion: { (errorMessage, data, token) in
                if let value = data {
                    let competitions: Competitions = Competition.models(data: value)
                    completion(competitions)
