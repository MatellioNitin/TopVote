...
extension Entry {
   ...
   static func getSingleEntry(entryId: String, error: @escaping (_ errorMessage: String) -> Void, completion: @escaping (_ entry: Entry) -> Void) {
    ...
    static func updateEntry(entryId: String, params: [String: Any], error: @escaping (_ errorMessage: String) -> Void, completion: @escaping () -> Void) {
        Entry.provider.request(Entry.API.updateEntry(params: params, entryId: entryId)) { result in
            result.handleResponseData(completion: { (errorMessage, data, token) in
            if let value = data{
                    completion()
            }
